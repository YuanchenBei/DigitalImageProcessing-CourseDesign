...
def mean_filter(init_image_path,kernel_size):
    '''
    均值滤波器
            ...
            for i in range(-(kernel_size[0]//2),(kernel_size[0]//2)+1):
                for j in range(-(kernel_size[1]//2),(kernel_size[1]//2)+1):
                    ...
                    if now_x<0 or now_x>=init_image_mat.shape[0] or now_y<0 or now_y>=init_image_mat.shape[1]:
                        continue
                    weight_sum+=init_image_mat[now_x][now_y]
            #标准化及更新
            filtered_image_mat[x][y]=int(round(weight_sum/(kernel_size[0]*kernel_size[1])))
...
if __name__=='__main__':
    ...
    kernel_size=[3,3]
    filtered_image=mean_filter(init_image_path,kernel_size)
